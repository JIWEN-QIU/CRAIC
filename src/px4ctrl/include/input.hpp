...
public:
  ...
  ros::Time rcv_stamp;
  ros::Time start_time;  // 记录节点启动时间
  bool start_time_initialized{false};  // 标记启动时间是否已初始化

  Battery_Data_t();
  void feed(sensor_msgs::BatteryStateConstPtr pMsg, const Parameter_t &param);
};
...
public:
  ...
  LaserGround_t();
  void feed(std_msgs::Int32MultiArrayConstPtr pMsg);
};
