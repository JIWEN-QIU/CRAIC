...
Desired_State_t::Desired_State_t()
...
Desired_State_t::Desired_State_t(const Odom_Data_t &odom)
	: p(odom.p),
	  v(Eigen::Vector3d::Zero()),
	  a(Eigen::Vector3d::Zero()),
	  j(Eigen::Vector3d::Zero()),
...
PX4CtrlFSM::PX4CtrlFSM(Parameter_t &param_) : param(param_)
{
	state = MANUAL_CTRL;
	hover_pose.setZero();
	hover_vel.setZero();
	hover_yaw_rate = 0.0;
	command_trigger_sent = false;
}
...
void PX4CtrlFSM::process()
{
	...
	case MANUAL_CTRL:
	{
		if (rc_data.enter_hover_mode) // Try to jump to AUTO_HOVER
		{
			...
			ROS_INFO("\033[36m[px4ctrl] MANUAL_CTRL(L1) --> AUTO_HOVER(L2)\033[0m");
		}
		else if (param.takeoff_land.enable && takeoff_land_data.triggered && takeoff_land_data.takeoff_land_cmd == quadrotor_msgs::TakeoffLand::TAKEOFF) // Try to jump to AUTO_TAKEOFF
		{
			...
			if (rc_is_received(now_time)) // Check this only if RC is connected.
			{
				if (!rc_data.is_hover_mode || !rc_data.is_command_mode || !rc_data.check_centered())
				{
					...
					while (ros::ok())
					{
						...
						if (rc_data.is_hover_mode && rc_data.is_command_mode && rc_data.check_centered())
						{
							ROS_INFO("\033[36m[px4ctrl] OK, you can takeoff again.\033[0m");
							break;
						}
					}
					break;
			...
			ROS_INFO("\033[36m[px4ctrl] MANUAL_CTRL(L1) --> AUTO_TAKEOFF\033[0m");
		}
	...
	case AUTO_HOVER:
	{
		...
		{
			ROS_WARN("[px4ctrl] Exit AUTO_HOVER reason: is_hover=%d odom_recent=%d mode=%.2f gear=%.2f offboard=%d",
			...
			ROS_WARN("\033[33m[px4ctrl] AUTO_HOVER(L2) --> MANUAL_CTRL(L1)\033[0m");
		}
		else if (rc_data.is_command_mode && cmd_is_received(now_time))
		{
			if (state_data.current_state.mode == "OFFBOARD")
			{
				state = CMD_CTRL;
				des = get_cmd_des();
				ROS_INFO("\033[33m[px4ctrl] AUTO_HOVER(L2) --> CMD_CTRL(L3)\033[0m");
			}
		}
		else if (takeoff_land_data.triggered && takeoff_land_data.takeoff_land_cmd == quadrotor_msgs::TakeoffLand::LAND)
		{
			...
			ROS_INFO("\033[36m[px4ctrl] AUTO_HOVER(L2) --> AUTO_LAND\033[0m");
		}
		else
		{
			set_hov_with_rc();
			...
			ROS_INFO_THROTTLE(1.0,
							  "[px4ctrl] AUTO_HOVER waiting command. mode=%.2f gear=%.2f is_command=%d cmd_recent=%d offboard=%d",
							  rc_data.mode,
							  rc_data.gear,
			...
			if (state_data.current_state.mode != "OFFBOARD")
			{
				ROS_WARN_THROTTLE(1.0, "[px4ctrl] AUTO_HOVER is waiting for OFFBOARD, retry SetMode.");
				toggle_offboard_mode(true);
			}
			...
			else if (!command_trigger_sent && state_data.current_state.mode == "OFFBOARD")
			{
				command_trigger_sent = true;
				publish_trigger(odom_data.msg);
				ROS_INFO("\033[33m[px4ctrl] TRIGGER sent, allow user command.\033[0m");
			}
			else if (takeoff_land.delay_trigger.first && now_time > takeoff_land.delay_trigger.second)
			{
	...
	case CMD_CTRL:
	{
		if (!rc_data.is_hover_mode || !odom_is_received(now_time))
		{
			...
			ROS_WARN("\033[33m[px4ctrl] From CMD_CTRL(L3) to MANUAL_CTRL(L1)!\033[0m");
		}
		else if (!rc_data.is_command_mode || !cmd_is_received(now_time))
		{
			state = AUTO_HOVER;
			...
			set_hov_with_odom();
			des = get_hover_des();
			ROS_INFO("\033[36m[px4ctrl] From CMD_CTRL(L3) to AUTO_HOVER(L2)!\033[0m");
		}
		else
		{
			des = get_cmd_des();
	...
	case AUTO_TAKEOFF:
	{
		...
		{
			ROS_WARN_THROTTLE(1.0, "[px4ctrl] AUTO_TAKEOFF is waiting for ARM, retry OFFBOARD and ARM.");
			if (state_data.current_state.mode != "OFFBOARD")
			{
				toggle_offboard_mode(true);
			}
		...
		else if (odom_data.p(2) >= (takeoff_land.start_pose(2) + param.takeoff_land.height)) // reach the desired height
		{
			...
			command_trigger_sent = false;
			set_hov_with_odom();
			ROS_INFO("\033[36m[px4ctrl] AUTO_TAKEOFF --> AUTO_HOVER(L2)\033[0m");
	...
	case AUTO_LAND:
	{
		if (!rc_data.is_hover_mode || !odom_is_received(now_time))
		{
			...
			ROS_WARN("\033[33m[px4ctrl] From AUTO_LAND to MANUAL_CTRL(L1)!\033[0m");
		}
		else if (!rc_data.is_command_mode)
		{
			...
			set_hov_with_odom();
			des = get_hover_des();
			ROS_INFO("\033[36m[px4ctrl] From AUTO_LAND to AUTO_HOVER(L2)!\033[0m");
		}
		else if (!get_landed())
		{
			des = get_takeoff_land_des(-param.takeoff_land.speed);
		...
		else
		{
			...
			if (print_once_flag)
			{
				ROS_INFO("\033[36m[px4ctrl] Wait for about 10s to let the drone arm.\033[0m");
			...
			if (extended_state_data.current_extended_state.landed_state == mavros_msgs::ExtendedState::LANDED_STATE_ON_GROUND) // PX4 allows disarm after this
			{
				...
				if (now_time.toSec() - last_trial_time > 1.0)
				{
					if (toggle_arm_disarm(false)) // disarm
					{
						...
						ROS_INFO("\033[33m[px4ctrl] AUTO_LAND --> MANUAL_CTRL(L1)\033[0m");
					}
	...
	// STEP2: publish control commands to mavros
	...
	{
		ROS_WARN_THROTTLE(1.0, "[px4ctrl] KILLMOTOR is ignored in PVA-only mode.");
	}
	else
	{
		if
		...
		{
			ROS_WARN_THROTTLE(1.0, "[px4ctrl] TRUST_ADJUST_BY_LASER is ignored in PVA-only mode.");
		}
#endif

		publish_pva_ctrl(des, now_time);
#ifdef USE_LASER		
	}
#endif
	// STEP3: Detect if the drone has landed
...
void PX4CtrlFSM::set_hov_with_rc()
{
	if (param.manual_ctrl_mode == 0)
	{
		...
		hover_pose(2) += rc_data.ch[2] * param.max_manual_vel_z * delta_t * (param.rc_reverse.throttle ? 1 : -1);
		hover_pose(3) = normalize_angle(
			hover_pose(3) + rc_data.ch[3] * param.max_manual_yaw_rate * delta_t * (param.rc_reverse.yaw ? 1 : -1));
	}
	else
	{
		ros::Time now = ros::Time::now();
		...
		hover_vel(0) = rc_data.ch[1] * param.max_manual_vel * (param.rc_reverse.pitch ? 1 : -1);
		hover_vel(1) = rc_data.ch[0] * param.max_manual_vel * (param.rc_reverse.roll ? 1 : -1);
		hover_vel(2) = rc_data.ch[2] * param.max_manual_vel_z * (param.rc_reverse.throttle ? 1 : -1);
		hover_yaw_rate = 0.0;

		if (rc_data.stick_moving)
		{
			hover_pose.head<3>() = odom_data.p + hover_vel * param.manual_ctrl_lookahead_time;
		}
		hover_pose(3) = normalize_angle(
			hover_pose(3) + rc_data.ch[3] * param.max_manual_yaw_rate * delta_t * (param.rc_reverse.yaw ? 1 : -1));
	}
...
void PX4CtrlFSM::publish_pva_ctrl(const Desired_State_t &des, const ros::Time &stamp)
{
	...
	msg.velocity.x = des.v(0);
	msg.velocity.y = des.v(1);
	msg.velocity.z = des.v(2);

	msg.acceleration_or_force.x = des.a(0);
	msg.acceleration_or_force.y = des.a(1);
	msg.acceleration_or_force.z = des.a(2);

	msg.yaw = des.yaw;
	msg.yaw_rate = des.yaw_rate;

	ctrl_pva_pub.publish(msg);
}
