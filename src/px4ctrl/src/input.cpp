...
void RC_Data_t::feed(mavros_msgs::RCInConstPtr pMsg)
{
    ...
    stick_moving = (fabs(ch[0]) > 1e-5 || fabs(ch[1]) > 1e-5 || fabs(ch[2]) > 1e-5 || fabs(ch[3]) > 1e-5);
...
void Battery_Data_t::feed(sensor_msgs::BatteryStateConstPtr pMsg, const Parameter_t &param)
{
    ...
    // Linear estimation based on cell_full_voltage, cell_cutoff_voltage and series
    double full_voltage = param.battery.series * param.battery.cell_full_voltage;
    double cutoff_voltage = param.battery.series * param.battery.cell_cutoff_voltage;
    if (full_voltage > cutoff_voltage)
    {
        percentage = (volt - cutoff_voltage) / (full_voltage - cutoff_voltage);
        percentage = std::max(0.0, std::min(1.0, percentage)); // Clamp to [0, 1]
    }
    else
    {
        percentage = pMsg->percentage; // Fallback to message percentage
    }

    // 只有在启动10秒后才打印电量信息，避免启动时的缓存问题
    if ((rcv_stamp - start_time).toSec() > 10.0)
    {
        ...
        if (percentage > 0.05)
        {
            ...
            {
                if (percentage >= 0.5) {
                    ROS_INFO("\033[32m[px4ctrl] Voltage=%.3f, percentage=%.3f\033[0m", volt, percentage);
                } else if (percentage >= 0.25) {
                    ROS_INFO("\033[33m[px4ctrl] Voltage=%.3f, percentage=%.3f\033[0m", volt, percentage);
                } else {
                    ROS_INFO("\033[31m[px4ctrl] Voltage=%.3f, percentage=%.3f\033[0m", volt, percentage);
                }
        ...
        else
        {
            ...
            {
                ROS_ERROR("\033[31m[px4ctrl] Dangerous! voltage=%.3f, percentage=%.3f\033[0m", volt, percentage);
                last_print_t = rcv_stamp;
            }
        }
    }
