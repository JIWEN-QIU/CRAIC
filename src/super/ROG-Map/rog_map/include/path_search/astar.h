...
namespace path_search {
    ...
    private:
        ...
        ros::Subscriber debug_type_sub_;
    ...
    public:
        ...
        void setFineInfNeighbors(const vec_Vec3i &nei) {
            neighbor_list = nei;
            have_neighbor_list = true;
        }

        RET_CODE setup(const Vec3f &start_pt, const Vec3f &goal_pt, const int &flag,
                       const double &searching_horizon = 9999);

        void debugTypeCallback(const std_msgs::Int32::ConstPtr &msg) {
            int type = msg->data;
            if (type == 0) {
                cfg_.visual_process = true;
            } else {
                cfg_.visual_process = false;
            }
        }
        ...
        void initMap(const ros::NodeHandle &nh, ROGMap::Ptr rm) {// TODO
            ...
            map_ptr_ = rm;
            debug_type_sub_ = nh_.subscribe("/debug_type", 1, &Astar::debugTypeCallback, this);
                        ...
                        for (int dz = -1; dz <= 1; dz++) {
                            ...
                            if (md_.use_inf_map) {
                                ...
                                if (!md_.use_inf_neighbor) {
                                    ...
                                    if (md_.unknown_as_occ && neighbor_type == UNDEFINED) {
...
//                    printf("\033[34m Escape: A star iter:%d, time:%.3f ms\033[0m\n", num_iter, (time_2 - time_1).toSec() * 1000);
        ...
        bool findNearestNeighborIs(const GridType &type, const Vec3f &src_pt, Vec3f &output_pt) {
            GridType start_type;
        ...
        bool findNearestNeighborNot(const GridType &type, const Vec3f &src_pt, Vec3f &output_pt) {
            GridType start_type;
            Vec3f cand_pt = src_pt;
            for (int i = 0; i < sorted_pts.size(); i++) {
                if (!insideLocalMap(cand_pt)) {
        ...
        bool neighborHaveOne(const GridType &type, const Vec3i &src_id) {
            for (const auto &nei: neighbor_list) {
                Vec3i nei_id = src_id + nei;
