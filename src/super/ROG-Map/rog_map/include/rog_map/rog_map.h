...
namespace rog_map {
    ...
    private:
        ...
        struct ROSCallback {
            ...
            ros::Subscriber *othertraj_sub;
            int unfinished_frame_cnt{0};
            Pose pc_pose;
            PointCloud pc;
            ros::Timer update_timer, mutual_avoid_timer;
            mutex updete_lock;

            std::map<ros::Subscriber*, int> subscriber_topic_map;
            std::vector<geometry_utils::Trajectory> other_trajs, last_other_trajs;
            std::vector<bool> othertraj_changed_flag;
            vec_E<vec_E<Vec3f>> last_other_traj_points;
            ros::Subscriber outofstuck_sub;
            // ros::Subscriber drone_state_sub_;
            vec_E<Vec3f> other_drone_pos;
        } rc_;
    ...
    private:
        ...
        void OtherTrajCallback(const quadrotor_msgs::PolynomialTrajectoryConstPtr &traj_msg, const int& drone_id);

        // void droneStateMsgCallback(const exploration_manager::DroneStateConstPtr& msg);

        void MutualAvoidCallback(const ros::TimerEvent &event);

        void outOfStuckCallback(const std_msgs::EmptyConstPtr& msg);
