...
namespace path_search {
    ...
    Astar::setup(const Vec3f &start_pt, const Vec3f &goal_pt, const int &flag, const double &searching_horizon) {
        ...
        if (md_.use_inf_neighbor && md_.use_inf_map) {
            throw std::runtime_error("Astar::setup: USE_INF_NEIGHBOR and ON_INF_MAP cannot be used together");
        }
        if (md_.use_inf_neighbor && !have_neighbor_list) {
            throw std::runtime_error("Astar::setup: USE_INF_NEIGHBOR need to have neighbor list");
        }
