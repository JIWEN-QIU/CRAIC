...
void ROSParamLoader::resetMapSize(ROGMapConfig &cfg) {
    ...
    int inflation_ratio = std::round(cfg.inflation_resolution / cfg.resolution);
    if(inflation_ratio%2 == 0) inflation_ratio +=1;
...
ROSParamLoader::ROSParamLoader(const ros::NodeHandle &nh, ROGMapConfig &cfg, const string &name_space) : nh_(nh) {
    ...
    if (cfg.swarm_en) {
        LoadParam(name_space + "/drone_id", cfg.drone_id, 1);
        LoadParam(name_space + "/drone_num", cfg.drone_num, 1);
        LoadParam(name_space + "/other_traj_topics", cfg.other_traj_topics, string(""));
    }
...
//    ROS_ERROR("The inflation resolution is set to %f", cfg.inflation_resolution);
    ...
    vector<double> boundingbox_min, boundingbox_max;
    LoadParam(name_space + "/boundingbox_min", boundingbox_min, vector<double>{0, 0, 0});
    LoadParam(name_space + "/boundingbox_max", boundingbox_max, vector<double>{0, 0, 0});
    double sum_min = 0, sum_max = 0;
    for (int i = 0; i < 3; i++) {
        sum_min += boundingbox_min[i];
        sum_max += boundingbox_max[i];
    }
    bool boundingbox_valid = true;
    for (int i = 0; i < 3; i++) {
        if (boundingbox_min[i] >= boundingbox_max[i]) {
            boundingbox_valid = false;
            break;
        }
    }
    if (boundingbox_min.size() != 3 || boundingbox_max.size() != 3 || (sum_min == 0.0 && sum_max == 0.0) || cfg.map_sliding_en || !boundingbox_valid) {
        ROS_WARN("Bounding box size is not 3! Not use it");
        // exit(-1);
    }else{
        // use bounding box to set map size and origin
        double inf_map_size = 0.5;
        cfg.map_size_d = Vec3f(boundingbox_max[0] - boundingbox_min[0] + inf_map_size,
                               boundingbox_max[1] - boundingbox_min[1] + inf_map_size,
                               boundingbox_max[2] - boundingbox_min[2] + inf_map_size);
        cfg.fix_map_origin = Vec3f(boundingbox_min[0] + (boundingbox_max[0] - boundingbox_min[0]) / 2,
                                   boundingbox_min[1] + (boundingbox_max[1] - boundingbox_min[1]) / 2,
                                   boundingbox_min[2] + (boundingbox_max[2] - boundingbox_min[2]) / 2);
    }
