...
namespace rog_map {
    ...
    void InfMap::setOccupied(const Vec3f &pos)
    {
        if (!insideLocalMap(pos)) return;
        md_.occupied_cnt[getHashIndexFromPos(pos)] = 1;
        md_.occ_inflate_cnt[getHashIndexFromPos(pos)] = 1;
    }
...
//
    ...
    GridType InfMap::getGridType(const Vec3i &id_g) const {
        ...
        (!cfg_.unk_inflation_en && md_.unknown_cnt[addr] == md_.sub_grid_num)) {
            return UNKNOWN;
        } 
        else {
            return KNOWN_FREE;
