...
void ProbMap::setOccupied(const Vec3f &pos) {
    if (!insideLocalMap(pos)) {
        return;
    }
    
    occupancy_buffer_[getHashIndexFromPos(pos)] = cfg_.l_occ + 0.01;
    inf_map_->setOccupied(pos);

    // set all the neighbors to be occupied, use pos to loop, 0.1 is the resolution
    for (double dx = -0.1; dx <= 0.1; dx += 0.1) {
        for (double dy = -0.1; dy <= 0.1; dy += 0.1) {
            for (double dz = -0.1; dz <= 0.1; dz += 0.1) {
                Vec3f p(dx, dy, dz);
                if (p.norm() <= 0.1) {
                    Vec3f pp = pos + p;
                    int hash_id = getHashIndexFromPos(pp);
                    occupancy_buffer_[hash_id] = cfg_.l_occ + 0.01;
                }
            }
        }
    }
...
void ProbMap::updateProbMap(const PointCloud &cloud, const Pose &pose) {
    ...
    // trans pos to lidar center pos
    Eigen::Vector3f extrinsic;
    extrinsic << 0.022242, -0.008652, 0.128790;
    // from pose's second Quatf, compute rotation matrix
    Eigen::Matrix3f rotation;
    Eigen::Quaternionf quat(pose.second.w(), pose.second.x(), pose.second.y(), pose.second.z());
    rotation = quat.toRotationMatrix();
    extrinsic = rotation * extrinsic;
    pos.x() = pos.x() + extrinsic(0);
    pos.y() = pos.y() + extrinsic(1);
    pos.z() = pos.z() + extrinsic(2);

    time_consuming_[4] = cloud.size();

    // cout << " -- [ROGMapCore] Update map at pose: " << pos.transpose() << endl;
    ...
    // cout << " -- [ROGMapCore] after map sliding" << endl;
    ...
    // cout << " -- [ROGMapCore] after virtual ceil and ground check" << endl;
...
void ProbMap::raycastProcess(const PointCloud &input_cloud, const Vec3f &cur_odom) {
    ...
    for (const auto &pcl_p: input_cloud) {
        ...
        // 1.2) temporal filter
        if ((temperol_cnt++ % cfg_.point_filt_num) && p.norm() > 5.0){
            continue;
        }
        ...
        for (const auto &p: inf_cloud) {
            ...
            while (raycast_data_.raycaster.step(ray_pt)) {
                ...
                if (dis > cfg_.blind_filter_dis) {
                    ...
                    // Local Map
                    if ((p_in - raycast_box_min).minCoeff() < 0 ||
                        (p_in - raycast_box_max).maxCoeff() > 0) {
                        p_in = geometry_utils::lineBoxIntersectPoint(p, cur_odom, raycast_box_min,
                                                                     raycast_box_max);
                    }
