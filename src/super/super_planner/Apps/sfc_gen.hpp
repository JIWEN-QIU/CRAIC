...
inline void convertPointNormFormToPlanEquation(const Eigen::MatrixXd &hPoly,
                                               Eigen::MatrixX4d &planEquation) {
    planEquation.resize(hPoly.cols(), 4);
    planEquation.leftCols(3) = hPoly.topRows(3).transpose();
    planEquation.col(3) = (hPoly.topRows(3).array() * hPoly.bottomRows(3).array()).colwise().sum();
}
...
inline bool extractV(const Eigen::MatrixXd &hPoly,
    ...
    Eigen::MatrixX4d hPoly_4;
    convertPointNormFormToPlanEquation(hPoly, hPoly_4);
    return geometry_utils::enumerateVs(hPoly_4, vPoly);
}
...
inline std::vector<Eigen::MatrixXd> genPolySFC(const int &num,
    ...
    co = offset;
    inifin.col(0) = co;
    hPoly = genPolyH(number_of_faces, rmin, rmax);
