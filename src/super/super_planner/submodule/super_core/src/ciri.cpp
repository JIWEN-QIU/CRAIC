...
namespace super_planner {
    ...
    RET_CODE CIRI::comvexDecomposition(const Eigen::MatrixX4d &bd, const Eigen::Matrix3Xd &pc, const Eigen::Vector3d &a,
                                       const Eigen::Vector3d &b, const Ellipsoid &init_E) {
//        benchmark_utils::TimeConsuming tc("CIRI::comvexDecomposation");
//        cout<<"Pc size: "<<pc.cols()<<endl;
        ...
        Ellipsoid E(init_E.R(), init_E.r().normalized(), init_E.d());


        auto isOutOfPlane = [&](const Vec4f &plane, const Vec3f &p, const double &robot_r) -> bool {
            return (plane.head(3).dot(p) + plane(3)) / plane.head(3).norm() > robot_r - epsilon_;
        };
        ...
        for (int loop = 0; loop < iter_num_; ++loop) {
            ...
            for (int i = 0; !completed && i < (M + N); ++i) {
                if (minSqrD < minSqrR) {
                    ...
                    if (isOutOfPlane(temp_plane_w, a, 0)) {
                    ...
                    } else if (isOutOfPlane(temp_plane_w, b, 0)) {
                        // Case the plan make seed out, the plane should be modified in world frame
                ...
                for (int j = 0; j < N; ++j) {
                    if (pcFlags(j)) {
                        if (isOutOfPlane(temp_plane_w, pc.col(j), robot_r_)) {
                            pcFlags(j) = 0;
                        } else {
                            completed = false;
                            if (minSqrR > distRs(j)) {
            ...
            if (!MVIE::maxVolInsEllipsoid(hPoly, E)) {
                ...
                VisualUtils::VisualizeLine(debug_pub_, a, b, Color::Pink(), Color::Orange(), "seed_line");
                E.Visualize(debug_pub_, "seed_ellipsoid", Color::Red());
    ...
    RET_CODE CIRI::comvexDecomposition(const Eigen::MatrixX4d &bd, const Eigen::Matrix3Xd &pc, const Eigen::Vector3d &a,
        ...
        vector<Eigen::Vector4d> planes;
        MatD4f hPoly;
        Ellipsoid obstcal_sphere_template(Mat3f::Identity(), robot_r_ * Vec3f(1, 1, 1), Vec3f::Zero());
