...
bool BackupTrajOpt::processCorridor() {
    ...
    if (!geometry_utils::enumerateVs(opt_vars.hPolytope, curIV)) {
        std::cout << RED << " -- [MINCO] enumerateVs failed." << RESET << std::endl;
        return false;
    }
    long nv = curIV.cols();
    curIOB.resize(3, nv);
...
bool BackupTrajOpt::setupProblemAndCheck() {
    ...
    if (!processCorridor()) {
        std::cout << RED << " -- [processCorridor] Failed to get Overlap enumerateVs ." << RESET << std::endl;
        return false;
    }
...
double BackupTrajOpt::optimize(Trajectory &traj, const double &relCostTol) {
    ...
    opt_vars.iter_num = 0;
    benchmark_utils::TimeConsuming ttt(" -- [BackupTrajOpt]", false);
    if (opt_vars.debug_en) {
        ret = lbfgs::lbfgs_optimize(x,
                                    minCostFunctional,
                                    &BackupTrajOpt::costFunctional,
                                    nullptr,
                                    &BackupTrajOpt::visualizeProgress,
                                    &this->opt_vars,
                                    lbfgs_params);
    ...
    if ((cfg_.penna_pos > 0 && opt_vars.penalty_log(1) > 0.15) ||
        ...
        cout << "\tThr: " << opt_vars.penalty_log(7) << endl;
        cout << "\tTs: " << opt_vars.ts << endl;
        cout << RED << " -- [BackOpt] Opt failed, Omg or thr or Pos violation." << RESET << endl;
    }
    ...
    if (ret >= 0) {
        ...
        std::cout << RED << " -- [MINCO] TrajOpt failed, " << lbfgs::lbfgs_strerror(ret) << RESET << std::endl;
    }
    return minCostFunctional;
}

int
BackupTrajOpt::visualizeProgress(void *instance, const Eigen::VectorXd &x, const Eigen::VectorXd &g, const double fx,
                                 const double step, const int k, const int ls) {
    OptimizationVariables &obj = *(OptimizationVariables *) instance;
    /// only for debug
    static int iter_num = 0;
    using namespace std;
    Trajectory temp_traj;
    obj.minco.getTrajectory(temp_traj);
    cout << setprecision(9) << GREEN << "iter_num: " << iter_num++ << "; g_norm = " << g.norm()
         << RESET << endl;
    cout << g.transpose() << endl;
    cout << "======================================iter_num: " << iter_num << "======================================"
         << endl;
    cout << "Start_t: " << obj.ts << endl;
    cout << "Grad start t: " << obj.gradTs << endl;
    cout << "times: " << temp_traj.getDurations().transpose() << endl;
    cout << "Trajecotyr max vel rate:" << temp_traj.getMaxVelRate() << endl;
    cout << "Trajecotyr max acc rate:" << temp_traj.getMaxAccRate() << endl;
    cout << "Duration: " << temp_traj.getTotalDuration() << endl;
    cout << "head vel_norm: " << obj.exp_traj.getVel(obj.ts).norm() << endl;
    cout << "head acc: " << obj.exp_traj.getAcc(obj.ts).norm() << endl;
    /// only for debug visualization

    VisualUtils::DeleteMkrArr(obj.debug_pub);
    Vec3f start_pt = temp_traj.getPos(0.0);
    VisualUtils::VisualizePoint(obj.debug_pub, start_pt, Color::Blue(), "spli_pt", 0.15, 0);
    ros::Duration(0.01).sleep();
    temp_traj.Visualize(obj.debug_pub, "back", Color::Red(), 0.08, true, false);
    ros::Duration(0.01).sleep();
    return 0;
}

BackupTrajOpt::BackupTrajOpt(const ros::NodeHandle &nh, TrajOptConfig &cfg) {
    using namespace std;
    nh_ = nh;
    cfg_ = cfg;
    ...
    mkr_pub_ = nh_.advertise<visualization_msgs::MarkerArray>("traj_opt/back/mkr_arr", 1000);
...
bool BackupTrajOpt::checkTrajMagnitudeBound(Trajectory &out_traj) {
    if (cfg_.penna_vel > 0 && out_traj.getMaxVelRate() > 1.2 * cfg_.max_vel) {
        std::cout << RED << " -- [TrajOpt] Minco backup opt failed." << RESET << std::endl;
        std::cout << RED << "\t\tBackend Max vel:\t" << out_traj.getMaxVelRate() << " m/s" << RESET
                  << std::endl;
        VisualUtils::DeleteMkrArr(mkr_pub_);
        VisualUtils::VisualizeTrajectory(mkr_pub_, out_traj, Color::Red(), 0.03, "failed_back", 0.1);
        return false;
    }
    if (cfg_.penna_acc > 0 && out_traj.getMaxAccRate() > 1.2 * cfg_.max_acc) {
        std::cout << RED << " -- [TrajOpt] Minco backup opt failed." << RESET << std::endl;
        std::cout << RED << "\t\tBackend Max Acc:\t" << out_traj.getMaxAccRate() << " m/s" << RESET
                  << std::endl;
        VisualUtils::DeleteMkrArr(mkr_pub_);
        VisualUtils::VisualizeTrajectory(mkr_pub_, out_traj, Color::Red(), 0.03, "failed_back", 0.1);
        return false;
...
BackupTrajOpt::optimize(const Trajectory &exp_traj,
    ...
    if (isnan(opt_vars.hPolytope.sum())) {
        std::cout << RED << " -- [BackTrajOpt] Polytope is nan." << RESET << std::endl;
        return false;
    }
    ...
    if (!setupProblemAndCheck()) {
        std::cout << RED << " -- [TrajOpt] Minco corridor preprocess error." << RESET << std::endl;
        success = false;
    }

    if (success && std::isinf(optimize(out_traj, cfg_.opt_accuracy))) {
        std::cout << YELLOW << " -- [SVC] Minco backup_traj opt failed." << RESET << std::endl;
        success = false;
    }

    if (opt_vars.penalty_log(1) > cfg_.penna_pos * 0.05) {
        std::cout << YELLOW << " -- [SVC] Minco backup_traj out of corridor." << RESET << std::endl;
        success = false;
        VisualUtils::DeleteMkrArr(mkr_pub_);
        out_traj.Visualize(mkr_pub_, "failed_back_out", Color::Red(), 0.1);
        sfc.Visualize(mkr_pub_, "failed_back_out", true);
    }
    out_ts = opt_vars.ts;
