...
namespace traj_opt {
    ...
    void YawTrajOpt::getYawTimeAllocation(const double &duration, VecDf &times) {
    ...
    void YawTrajOpt::getYawWaypointAllocation(const Vec4f &init_state, Vec4f &goal_state, VecDf &way_pts, VecDf &times,
        ...
        double pos_traj_duration = pos_traj.getTotalDuration();
        for (long unsigned int i = 0; i < times.size() - 1; i++) {
            eval_t += times(i);
            double cur_yaw;
...
//                          " -- [SVC] Yaw planning failed, the goal yaw is too close to the current yaw.\n");
                cur_yaw = last_yaw;
            }
            way_pts(i) = cur_yaw;
            last_yaw = cur_yaw;
...
//            cout << times.transpose() << endl;
    ...
    bool YawTrajOpt::optimize(const Vec4f &istate_in,
        ...
        switch (order) {
            case 3: {
                Vec2f init_state3 = init_state.head(2);
                Vec2f goal_state3 = goal_state.head(2);
...
//            yaw_traj.printProfile();
        ...
        if (max_yaw_rate > yaw_dot_max_ + 2.0) {
            cout << RED << " Yaw rate too large, " << max_yaw_rate << RESET << endl;
//                return false;
        }
        out_traj = yaw_traj;
